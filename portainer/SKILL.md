...
metadata: {"picoclaw":{"emoji":"🐳","requires":{"bins":["curl","jq"],"credentials":"~/.picoclaw/workspace/.credentials/portainer.json"}}
---
...
> *"Docker containers? I'll handle them."* 🦀
...
2. **Configure credentials:**
   Create `~/.picoclaw/workspace/.credentials/portainer.json`:
   ```json
   {
     "url": "https://your-portainer-server:9443",
...
**Solution:** Create `~/.picoclaw/workspace/.credentials/portainer.json`:
```json
{
  "url": "https://your-server:9443",
  "apiKey": "ptr_your_token"
}
```
...
| 1.1.0 | 2026-02-16 | Picoclaw branding; credentials from `/workspace/.credentials/portainer.json` (JSON) |
...
```
  ...
  | |   | |
  | |___| |
   \_____/   "Containers are just fancy shells
