...
8. [Security Hardening](#security-hardening) (expanded, includes Credential Storage)
...
├── .credentials/      # Credentials for skills (never commit)
│   └── {skill-name}.json
└── memory/
...
**The Rule:** All credentials are stored in `/workspace/.credentials/{skill-name}.json`.

- Every skill that needs credentials **must** refer to the `.credentials` folder
- Path format: `/workspace/.credentials/{skill-name}.json` (e.g. `/workspace/.credentials/proactive-agent.json`)
- Never embed credentials in code, prompts, or memory files
- Never commit `.credentials/` — ensure it is in `.gitignore`
...
**Forked and modified by:** tommynurwantoro — free to modify; creator attribution preserved.
